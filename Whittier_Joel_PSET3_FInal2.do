...
replace sex = "Unknown" if missing(sex)
replace age = "Unknown" if missing(age)
replace security_deposit = 0 if missing(security_deposit)
replace cleaning_fee = 0 if missing(cleaning_fee)


gen zip_dig = floor(zipcode/10000) // this tells us the generic region
sort data_class zip_dig price

local rowiter 0
forvalues i = 0/9{
count if zip_dig == `i' & data_class ==1
local percent = floor(r(N)/10)
drop if _n < r(N)-`percent'+`rowiter' & _n > `rowiter'
local rowiter = `rowiter' + r(N)
}
...
gen num = 1 if number_of_reviews <= 5
...
regress review_scores_rating i.room_type_cat /// 
	host_listings_count ///
...
drop if data_class ==2 //ha ha fuck you data class 2 
replace lnum = 0 if missing(lnum)
replace descrip=0 if missing(descrip)
replace lextra_people = 0 if missing(lextra_people)
predict OLS
...
replace lextra_people = . if lextra_people==0
...
randomforest review_scores_rating old room_type_cat /// 
	...
	guests_included extra_people cavail bed_type_cat namen rules cleaning_fee ///
	instant_bookable lnum bigword day week month bpr num lextra_people lhost_response_rate stateg lbeds apt maxnights smoke bad_host in 1/`training_rows', type("reg") iter(100) lsize(20) numvars(12) depth(10)
...
preserve 
keep id OLS RF
export delimited "2_12110109_Whittier_Joel.csv", replace // export csv file with predictions 
restore
